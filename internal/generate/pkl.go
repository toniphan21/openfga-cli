...
type PklConventionConfig struct {
	...
	BaseListObjectsName string `json:"base_list_objects_name"`
	TypeName            string `json:"type_name"`
	AssertionsName      string `json:"assertions_name"`
	ListObjectsName     string `json:"list_objects_name"`
	...
	ListObjectsRelationFormat string `json:"list_objects_relation_format"`
...
func (g *PklGenerator) Generate() (*GeneratedFiles, error) {
	...
	gListObjects := g.collectGenListObjects(gAssignments, gAssertions)
	...
	g.generateBaseListObjects(&gListObjects, &gen)
	files = append(files, GeneratedFile{
		Path:             "lib/gen.pkl",
		Content:          gen.String(),
		OverrideIfExists: true,
	...
	var listsDefs strings.Builder
	listsDefs.WriteString(`import "lib/gen.pkl"`)
	listsDefs.WriteString("\n")
	g.generateListsDefinitionContent(&gListObjects, &listsDefs)
	files = append(files, GeneratedFile{
		Path:             "lists.pkl",
		Content:          listsDefs.String(),
		OverrideIfExists: false,
	})
...
func (g *PklGenerator) generateListsDefinitionContent(listObjects *[]genListObject, gen *strings.Builder) {
	for _, t := range *listObjects {
		baseClassName := g.Convention.getBaseListObjectsName(t.name)
		writeLine(gen, "")
		writeLine(gen, "class %v extends gen.%v {", g.Convention.getListObjectsName(t.name), baseClassName)
		writeLine(gen, "  // you can put some custom ListObjects in this class")
		writeLine(gen, "")
		writeLine(gen, "}")
	}
}
...
func (g *PklGenerator) generateBaseTypes(assignments *[]genAssignment, gen *strings.Builder) {
	for _, t := range *assignments {
		...
		listObjectsTypes := make(map[string]bool)
		...
		for _, k := range rels {
			...
			for _, o := range objs {
				...
				listObjectsTypes[obj.name] = true
			}
		...
		for target := range listObjectsTypes {
			writeLine(gen, "")
			writeLine(gen, "  function assert_list_objects_%v(", strcase.ToSnake(target))
			writeLine(gen, "    name: String,")
			writeLine(gen, "    customListObjects: %v,", g.Convention.getBaseListObjectsName(target))
			writeLine(gen, "    listObjects: (%v) -> Listing", g.Convention.getBaseListObjectsName(target))
			writeLine(gen, "  ): Mapping = new Mapping {")
			writeLine(gen, `    ["name"] = name`)
			writeLine(gen, `    ["list_objects"] = new Listing {`)
			writeLine(gen, "      new Mapping {")
			writeLine(gen, `        ["user"] = toFGAType()`)
			writeLine(gen, `        ["type"] = "%v"`, target)
			writeLine(gen, `        ["assertions"] = listObjects.apply(customListObjects)`)
			writeLine(gen, `          .fold((new Mapping {}).toMap(), (r: Map, m: Mapping) -> r + m.toMap())`)
			writeLine(gen, "      }")
			writeLine(gen, "    }")
			writeLine(gen, "  }")
...
func (g *PklGenerator) generateBaseListObjects(listObjects *[]genListObject, gen *strings.Builder) {
	for _, t := range *listObjects {
		baseType := g.Convention.getBaseTypeName(t.name)
		baseClassName := g.Convention.getBaseListObjectsName(t.name)
		...
		for _, rel := range t.relations {
			writeLine(gen, "")
			name := g.Convention.getListObjectsRelationName(t.name, rel)
			writeLine(gen, "  function %v(items: List<%v>) = new Mapping {", name, baseType)
			writeLine(gen, `    ["%v"] = items.map((item: %v) -> item.toFGAType()).toListing()`, rel, baseType)
			writeLine(gen, "  }")

			writeLine(gen, "")
			prefix := g.Convention.getAssertHasRelationPrefix(t.name)
			writeLine(gen, "  function %v%v(items: List<%v>) = new Mapping {", prefix, rel, baseType)
			writeLine(gen, `    ["%v"] = items.map((item: %v) -> item.toFGAType()).toListing()`, rel, baseType)
			writeLine(gen, "  }")
		}

		for _, rel := range t.accesses {
			writeLine(gen, "")
			name := g.Convention.getListObjectsAccessName(t.name, rel)
			writeLine(gen, "  function %v(items: List<%v>) = new Mapping {", name, baseType)
			writeLine(gen, `    ["%v"] = items.map((item: %v) -> item.toFGAType()).toListing()`, rel, baseType)
			writeLine(gen, "  }")
...
func (g *PklGenerator) collectGenListObjects(assignments []genAssignment, assertions []genAssertion) []genListObject {
	m := make(map[string]map[string]bool)
	for _, assertion := range assertions {
		_, ok := m[assertion.name]
		if !ok {
			m[assertion.name] = make(map[string]bool)
		}
		for _, rel := range assertion.relations {
			m[assertion.name][rel] = true
		}
	}

	for _, assignment := range assignments {
		for _, rel := range assignment.relations {
			for on := range rel.objects {
				_, ok := m[on]
				if !ok {
	...
	for _, key := range keys {
		...
		for rel, ok := range mm {
			if ok {
				accesses = append(accesses, rel)
			} else {
				relations = append(relations, rel)
			}
		}
		if len(accesses) > 0 || len(relations) > 0 {
			result = append(result, genListObject{
				name:      key,
				relations: relations,
				accesses:  accesses,
			})
...
func (c *PklConvention) getListObjectsName(userType string) string {
	if cf, ok := c.Config[userType]; ok && cf.ListObjectsName != "" {
		return cf.ListObjectsName
	}
	return strcase.ToCamel(userType) + "ListObjects"
}
...
func (c *PklConvention) getBaseListObjectsName(userType string) string {
	if cf, ok := c.Config[userType]; ok && cf.BaseListObjectsName != "" {
		return cf.BaseListObjectsName
	}
	return "Base" + strcase.ToCamel(userType) + "ListObjects"
}
...
func (c *PklConvention) getListObjectsRelationName(typ, rel string) string {
	f := ""
	if cf, ok := c.Config[typ]; ok && cf.ListObjectsRelationFormat != "" {
		f = c.allowNone(cf.ListObjectsRelationFormat)
	}
	if f == "" {
		f = "is_[relation]_of"
	}
	return strings.ReplaceAll(f, "[relation]", strcase.ToSnake(rel))
}

func (c *PklConvention) getListObjectsAccessName(typ, rel string) string {
	return c.getAssertPositiveForVerb(typ, rel)
}
